...
- [ ] Task: Create `RoleContext` and hook for global role state
    - [ ] Define `UserRole` type ('ADMIN', 'PLAYER').
    - [ ] Implement `RoleProvider` with state management and persistence (localStorage).
    - [ ] Create `useRole` hook for accessing role and update function.

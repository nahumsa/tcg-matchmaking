...
- **Landing Page:** A landing page at `/` to select between "Administrator" and "Player" if no role is currently selected.
- **Smart Redirection:**
    - If a role is already stored in `localStorage`, `/` should redirect to the appropriate view (Admin Dashboard or Participant Join).
...
- **Role Selection:** A dropdown menu in the navigation bar to select between "Administrator" and "Player" at any time.

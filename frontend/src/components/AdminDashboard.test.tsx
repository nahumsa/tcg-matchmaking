...
describe('AdminDashboard', () => {
  ...
  it('keeps the selected score when re-clicking the same outcome', async () => {
    const mockTournament = {
      id: 1,
      name: 'Swiss Open #1',
      code: 'ABCDEF',
    ...
    // Re-clicking the already selected outcome should not overwrite 0-2 to 1-2
    ...
    await waitFor(() => {
      const reportCalls = fetchMock.mock.calls.filter(([u, opts]) => u.toString().includes('/matches/10/report') && opts?.method === 'POST');
      expect(reportCalls.length).toBeGreaterThan(0);
      expect(reportCalls[0][1]?.body).toBe(JSON.stringify({ player1_score: 0, player2_score: 2, is_admin: true }));
    });
  });

...
def test_potential_pairings(setup_db):
    # Create tournament
    create_resp = client.post("/tournaments", json={"name": "Potential Pairings Tournament", "rounds": 3})
    code = create_resp.json()["code"]

    # Add 4 players
    p1 = client.post(f"/tournaments/{code}/join", json={"name": "Player 1"}).json()
    p2 = client.post(f"/tournaments/{code}/join", json={"name": "Player 2"}).json()
    p3 = client.post(f"/tournaments/{code}/join", json={"name": "Player 3"}).json()
    p4 = client.post(f"/tournaments/{code}/join", json={"name": "Player 4"}).json()
    ...
    for m in matches:
        ...
        client.post(f"/matches/{m['id']}/report", json={"player1_score": 2, "player2_score": 0})

    # Get points for P1
    p1_data = client.get(f"/tournaments/{code}/standings").json()
    p1_current = next(p for p in p1_data if p["id"] == p1["id"])
    
    # Potential pairings for P1 should not include who they just played
    # Find who P1 played
    m_p1 = next(m for m in matches if m["player1_id"] == p1["id"] or m["player2_id"] == p1["id"])
    played_opp_id = m_p1["player2_id"] if m_p1["player1_id"] == p1["id"] else m_p1["player1_id"]
    
    potential_resp = client.get(f"/tournaments/{code}/participants/{p1['id']}/potential-pairings")

...
from backend.app.pairing import get_pairings
...
def test_even_players_pairing():
    ...
    for pair in pairings:
        ...
        if p_a.id == 1: assert p_b.id == 2
        elif p_a.id == 2: assert p_b.id == 1
        elif p_a.id == 3: assert p_b.id == 4
        elif p_a.id == 4: assert p_b.id == 3
...
def test_avoid_repeat_matchups():
    ...
    for p_a, p_b in pairings:
        ...
        if p_a.id == 1: assert p_b.id != 2
        if p_b.id == 1: assert p_a.id != 2
        # P3 should NOT be paired with P4 again
        if p_a.id == 3: assert p_b.id != 4
        if p_b.id == 3: assert p_a.id != 4

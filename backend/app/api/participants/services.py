...
async def join_tournament(
    db: Session, tournament_id: int, code: str, participant: schemas.ParticipantJoin
) -> models.Participant:
    ...
    db_participant = models.Participant(
        name=participant.name, tournament_id=tournament_id
    )
...
async def remove_participant(
    db: Session, tournament_id: int, code: str, participant_id: int
):
...
def get_participants(db: Session, tournament_id: int) -> List[models.Participant]:
    ...
    return (
        db.query(models.Participant)
        .filter(models.Participant.tournament_id == tournament_id)
        .all()
    )
